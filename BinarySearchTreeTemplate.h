...
class BinarySearchTree {
 ...
 public:
  ...
  public:
     ...
     */
    iterator& operator++(){
      Node<KeyType, ValueType>* traversePointer = curr;
      if(traversePointer->getRight() == NULL){
        //if the current node doesn't have a right child, repeat going to parents until the parent is larger than currentNode
        KeyType biggestValueTraversed = curr->getKey();
        while(true){
          if(traversePointer->getParent() == NULL){ //if current iterator points at the largest emelment in the tree
            curr = traversePointer->getParent();
            break;
          }
          else if(traversePointer->getParent()->getKey() < biggestValueTraversed){
            traversePointer = traversePointer->getParent();
          }
          else{
            traversePointer = traversePointer->getParent();
            curr = traversePointer; 
            break;
          }
        }
      }
      else{
        //if the current node has a right child, return the child farest to the left of the right child.
        traversePointer = curr->getRight();
        while(traversePointer->getLeft()!=NULL){
          traversePointer = traversePointer->getLeft();
        }
        curr = traversePointer;
      }
      return *this;
   ...
   */
  iterator end(){
    return BinarySearchTree::iterator(NULL);
  }
